#!/bin/python3
# -*- coding: utf-8 -*-
import requests, re, configparser, telegram, os
from urllib import parse
from bs4 import BeautifulSoup
from moodle_db_create import Base, UUser, FFile, CCourse, BBlock
...
class Course:
	...
	def __Split(self):
		...
		bl=list()
		for block in blocks:
			b=Block(block.next_sibling, self._courseid, self._cookie)
			bl.append(b)
...
class Block:
	...
	def __AnalyseBlock(self):
		soup = self.__content
		if "activityinstance" in soup.contents[0].get('class', []):
			activityinstance=soup.contents[0]
			self.__type="url"
			...
			try:
				self._cont=soup.select(".contentafterlink")[0].text
			except (AttributeError, IndexError): 
				self._cont=""
			#Erstellen den Link-Objekts später, um wiederholte Downloads zu verhindern
		elif "contentwithoutlink" in soup.contents[0].get('class', []):
			self.__type="contentwithoutlink"
			self._url=""
			self._title=""
			self._cont=soup.contents[0].text
		else:
			self.__type="unknown"
			self._url=""
