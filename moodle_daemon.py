...
def CheckUser(bot, update):
    ...
    if not entry:
        # Nutzer ist neu
        new_user = UUser(id=chat.id, first_name=chat.first_name, last_name=chat.last_name, username=chat.username, title=chat.title, notifications=True, semester=default_semester, counter=0)
        session.add(new_user)
        session.commit()
        new_usr = copy.deepcopy(new_user)
        message = "Dieser Bot bietet Zugriff auf alle Moodle-Dateien zum Bachelor Elektro- und Informationstechnik ab WS16/17."
...
def ShowCourseContent(bot, update, arg):
    ...
    if not not entry:
        button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("🔍 Kurse", callback_data="1"), InlineKeyboardButton("🎞️ Videos", callback_data="6$" + arg)]]
    else:
        button_list = [[InlineKeyboardButton("🏠 Home", callback_data="0"), InlineKeyboardButton("🔍 Kurse", callback_data="1")]]
    reply_markup = InlineKeyboardMarkup(button_list)
...
def ShowVideoContent(bot, update, arg):
    ...
    if len(entries) > 0:
        message = {0:"Videos zu [" + entries[0].coursedata.name + "](https://www.moodle.tum.de/course/view.php?id=" + str(entries[0].course) + "): \n"}
    else:
...
def ShowHome(bot, update, usr, text="🏠 Home"):
    button1 = InlineKeyboardButton("🛡️ Benachrichtigungen deaktivieren", callback_data="5$0")  if usr.notifications else InlineKeyboardButton("📡 Benachrichtigungen aktivieren", callback_data="5$1")  
    button_list = [[button1], [InlineKeyboardButton("📆 Semester auswählen", callback_data="4")], [InlineKeyboardButton("🔍 Kurse anzeigen", callback_data="1")]]
    reply_markup = InlineKeyboardMarkup(button_list)
